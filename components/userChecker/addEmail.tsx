...
export const AddEmail = () => {
    ...
    async function handleEmailSetup(){
        try{
            await axios.patch("/api/user/"+user?.email, {wallet: newWallet}).then((res)=>{
                getUser()
            })
        }
        catch(err){
  ...
  return (
    <div className="w-screen h-screen z-[10000000000000] fixed top-0 left-0 backdrop-blur-xl flex flex-col items-center justify-center" >
        <div className='w-96  bg-white shadow-xl shadow-black/30 rounded-lg p-5'>
            ...
            <h2 className='text-sm text-gray-400 mt-5'> Kindly register your <b>Wallet ID</b> which will be used to link accounts when <b>logging in across different platforms.</b></h2>
            <input placeholder="Enter Wallet..." onChange={(e) => { setNewWallet(e.target.value); }} value={newWallet} className={`p-2 placeholder:text-gray-300 my-2 w-full peer focus:outline-none  focus:border-black focus:border-2 rounded-xl border-[1px] duration-200 `}></input>
