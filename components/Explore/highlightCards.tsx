...
export default function HighlightCards({ highlight, session, getUser }: { highlight: any, session: any, getUser: () => void }) {
    ...
    return (
        <div className='md:w-[450px] max-md:w-[20rem] max-md:h-[25rem] p-4 bg-gray-200 flex flex-row max-md:flex-col items-center justify-start overflow-hidden relative rounded-xl'>
            ...
            <div className='flex flex-row gap-2 max-md:items-center max-md:justify-center md:absolute bottom-8 right-8 z-20'>
                ...
                {session && <button disabled={highlight.readlisted} onClick={() => { readlist(highlight.item._id) }} className='text-nifty-black text-sm font-semibold bg-nifty-black rounded-lg w-8 h-8 flex items-center justify-center'>
