    ...
    <div className={`w-screen backdrop-blur-xl absolute left-0 flex flex-col items-center top-0 justify-start duration-200 transition-all overflow-hidden ${bringSearchBar ? "z-50 h-screen opacity-100": "z-[-50] h-0 opacity-0"} `} >
        ...
        <div className='flex flex-col w-[50%] max-md:w-[90%] bg-black/30 mt-2 rounded-xl'>
            ...
            {searchResults.map((item)=>(
                <button onClick={()=>{setHistoryData("U"+item.email) ;setIsLoading(true);router.push("/authors/"+item.wallet)}} className='px-4 hover:bg-white/20 duration-200 py-4 text-white border-b-2 border-white/50 shadow-xl shadow-black/30 rounded-b-xl relative z-50' >
        ...
        <div className='flex flex-col w-[50%] max-md:w-[90%] bg-black/30 mt-2 rounded-xl'>
            ...
            {bookHistory.map((item)=>(
                <button onClick={()=>{setHistoryData("B"+item._id) ;setIsLoading(true);router.push("/books/"+item._id)}} className='px-4 hover:bg-white/20 duration-200 py-4 text-white border-b-2 border-white/50 shadow-xl shadow-black/30 rounded-b-xl relative z-50' >
                    <div className='flex items-center justify-start gap-2'>
                        {/* {console.log(item.profileImage)} */}
                        {/* @ts-ignore */}
        ...
        <div className='flex flex-col w-[50%] max-md:w-[90%] bg-black/30 mt-2 rounded-xl'>
            ...
            {historyBookResults.map((item)=>(
                <button onClick={()=>{setHistoryData("B"+item._id) ;setIsLoading(true);("/books/"+item._id)}} className='px-4 hover:bg-white/20 duration-200 py-4 text-white border-b-2 border-white/50 shadow-xl shadow-black/30 rounded-b-xl relative z-50' >
                    <div className='flex items-center justify-start gap-2'>
                        {/* {console.log(item.profileImage)} */}
                        {/* @ts-ignore */}
