...
import { FaBookOpen, FaCrown, FaInfinity, FaLocationArrow, FaPause, FaPlay } from 'react-icons/fa';
  ...
  useEffect(() => {
    ...
    if (song?.duration) {
      if (progress == "w-[0%]") {
        setInterval(() => {
          //@ts-ignore
          const percent = ((song?.currentTime / song?.duration) * 15).toFixed(1);
          ...
          <div className='flex max-md:flex-col gap-8 object-center items-center max-md:py-10 md:h-full h-full md:px-10 w-screen justify-center md:justify-start my-auto absolute z-50 backdrop-blur-xl'>
            ...
            <div className='flex flex-col max-md:items-center max-md:justify-center gap-3 md:w-[50%] max-md:w-[90%] '>
              ...
              <div className='flex gap-2'>
                <button className='w-32 h-10 py-1 px-3 flex items-center justify-center rounded-lg text-white font-bold hover:-translate-y-1 duration-200 bg-black' onClick={() => { setLocalStorage(); increaseReader(bookDetails?.readers as number, bookDetails?._id) }}>Read</button>
                {/* @ts-ignore */}
                <button disabled={(bookDetails?.maxMint > 0 && bookDetails?.maxMint == bookDetails?.minted) || bookDetails?.isPaused} onClick={() => { setShowModal(true) }} className='text-black bg-gray-200 h-10 w-32 font-bold rounded-lg hover:-translate-y-1 px-3 py-1 transform transition duration-200 ease-in-out flex items-center justify-center flex-col gap-0'>{bookDetails?.isPaused && <h2 className='flex gap-2 items-center justify-center'>Paused <FaPause /></h2>}{bookDetails?.maxMint > 0 && bookDetails?.minted < bookDetails?.maxMint && !bookDetails?.isPaused && "Mint"} {bookDetails?.maxMint > 0 && bookDetails?.minted >= bookDetails?.maxMint && "Minted Out!"} {bookDetails?.maxMint == 0 && !bookDetails?.isPaused && "Collect"}</button>
